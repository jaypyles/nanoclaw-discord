...
function buildVolumeMounts(
  ...
  if (fs.existsSync(envFile)) {
    ...
    if (filteredLines.length > 0) {
      ...
      logger.warn(
        'No CLAUDE_CODE_OAUTH_TOKEN or ANTHROPIC_API_KEY in .env - container will run without Claude auth and may exit with code 1. Add one of these to .env and restart.',
      );
    }
  } else {
    logger.warn(
      'No .env file found - container will run without Claude auth. Create .env with CLAUDE_CODE_OAUTH_TOKEN or ANTHROPIC_API_KEY.',
    );
  }
